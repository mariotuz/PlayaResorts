...
<% Using Ajax.BeginForm("CatalogGroupType", "Catalogs", New AjaxOptions With {.OnBegin = "OnBegin", .OnComplete = "OnComplete", .OnSuccess = "OnSuccessEditRequest", .OnFailure = "OnFailureEditRequest", .UpdateTargetId = "AJAX_Container2"})%>
 ...
 <div id="toolbar">
    <%
        If (ViewData("idseccion4subseccion4edit_permission") IsNot Nothing And ViewData("idseccion4subseccion4edit_permission") > 0) Then
            Response.Write("<button id=""EditRequestGroupType""  name=""GroupTypeButtons"" value=""Edit_Request_GroupType"" class=""btn btn-default"" ><i class=""fa fa-pencil-square-o""></i> Edit</button>")
        End If
        
        If (ViewData("idseccion4subseccion4delete_permission") IsNot Nothing And ViewData("idseccion4subseccion4delete_permission") > 0) Then
            Response.Write("<a id=""DeleteRequestGroupType""  class=""btn btn-default"">Delete</a>")
        End If
    %>
