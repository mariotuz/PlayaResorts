...
<% Using Ajax.BeginForm("CatalogDistribution", "Catalogs", New AjaxOptions With {.OnBegin = "OnBegin", .OnComplete = "OnComplete", .OnSuccess = "OnSuccessEditRequest", .OnFailure = "OnFailureEditRequest", .UpdateTargetId = "AJAX_Container2"})%>
 ...
 <div id="toolbar">
     <%
         If (ViewData("idseccion4subseccion12edit_permission") IsNot Nothing And ViewData("idseccion4subseccion12edit_permission") > 0) Then
             Response.Write("<button id=""EditRequestDistribution""  name=""DistributionGroupsButtons"" value=""Edit_Request_Distribution"" class=""btn btn-default"" ><i class=""fa fa-pencil-square-o""></i> Edit</button>")
         End If
                                            
         If (ViewData("idseccion4subseccion12delete_permission") IsNot Nothing And ViewData("idseccion4subseccion12delete_permission") > 0) Then
             Response.Write("<a id=""DeleteRequestDistribution""  class=""btn btn-default"">Delete</a>")
         End If
    %>
