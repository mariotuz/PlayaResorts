        ...
        $.each(this.options.columns, function (i, column) {
            ...
            if (column.checkbox) {
                if (!that.options.singleSelect && that.options.checkboxHeader) {
                    text = '<input name="btSelectAll" id="custo-check" type="checkbox"/><label class="check" for="custo-check"></label>';
                }
            ...
            if (column.filterControl !== undefined) {
                switch (column.filterControl.toLowerCase()) {
                    case 'input':
                        ...
                        break;
                    case 'select':
                        html.push(sprintf('<select class="%s form-control slc-custom" style="width: 100%; visibility: %s"></select>',
    ...
    BootstrapTable.prototype.initToolbar = function () {
        ...
        if (this.options.showColumns) {
            html.push(sprintf('<div class="keep-open btn-group" title="%s">',
                ...
                sprintf('<i class="%s %s"> Colums</i>', this.options.iconsPrefix, this.options.icons.columns),
